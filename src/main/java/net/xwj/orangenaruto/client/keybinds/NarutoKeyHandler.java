package net.xwj.orangenaruto.client.keybinds;
...
import net.xwj.orangenaruto.client.gui.JutsuScreen;
...
public class NarutoKeyHandler {
    ...
    private static final String NARUTO_KEY_CATEGORY = "orangenaruto.keys.category";
    ...
    private static KeyBindingTickHeld JUTSU_R_KEY;
    private static KeyBindingTickHeld JUTSU_G_KEY;
    private static KeyBindingTickHeld JUTSU_Y_KEY;

    private static KeyBindingTickHeld JUTSU_MENU_KEY;

    /**
     * Attached on the main orangenaruto file to the mod bus as this isn't a forge bus event
     * @param event
     */
    @SubscribeEvent
    public static void registerKeyBinds(RegisterKeyMappingsEvent event) {
        LEAP_KEY = registerKeyBind("naruto.keys.leap", KeyEvent.VK_R, event);
        JUTSU_R_KEY = registerKeyBind("naruto.keys.key1", KeyEvent.VK_Y, event);
        JUTSU_G_KEY = registerKeyBind("naruto.keys.key2", KeyEvent.VK_I, event);
        JUTSU_Y_KEY = registerKeyBind("naruto.keys.key3", KeyEvent.VK_G, event);
        JUTSU_MENU_KEY = registerKeyBind("naruto.keys.jutsu_menu", KeyEvent.VK_J, event);

        JUTSU_KEYS.add(LEAP_KEY);
        JUTSU_KEYS.add(JUTSU_R_KEY);
        JUTSU_KEYS.add(JUTSU_G_KEY);
        JUTSU_KEYS.add(JUTSU_Y_KEY);
        ...
        JUTSU_R_KEY.registerClickConsumer(() -> handleJustuKey(1));
        JUTSU_G_KEY.registerClickConsumer(() -> handleJustuKey(2));
        JUTSU_Y_KEY.registerClickConsumer(() -> handleJustuKey(3));
    }
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if(lastKey > 0 && lastKey < JUTSU_KEYS.size()) {
            ...
            //orangenaruto.LOGGER.info("Key " + lastKey + "key" + key.getName() + " is held: " + key.isCurrentlyHeld() + " held: " + key.heldTicks + " isDown: " + key.isDown());
        ...
        //orangenaruto.LOGGER.info("Value is " + (currentJutsuCombo % 10));
    ...
    private static void checkNonCharging() {
        if(ticksSinceLastKey > NarutoConfig.jutsuCastDelay) {
            //orangenaruto.LOGGER.info("Would cast jutsu {}", currentJutsuCombo);
            Minecraft mc = Minecraft.getInstance();
            if(mc.player != null ) {
